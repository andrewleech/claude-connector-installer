"""A subset of CPython's winreg module, implemented via raw advapi32.dll FFI
calls (this MicroPython build has no built-in winreg/ctypes).

Covers create/open/close, get/set of a value, and delete of a key/value.
Not covered (natural follow-ups, not needed by any current consumer):
EnumKey/EnumValue/FlushKey, REG_DWORD_BIG_ENDIAN/REG_QWORD pack-unpack, and
mapping Win32 error codes onto specific exception subclasses the way
CPython does (e.g. ERROR_FILE_NOT_FOUND -> FileNotFoundError) -- errors here
are plain OSError(win32_error_code).
"""
